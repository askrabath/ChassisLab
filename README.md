...
## Four real-part representation variants

```sh
/private/tmp/chassis-lab-venv/bin/python -m chassis_lab.part_assemblies
# Rebuild saved variants without API calls:
/private/tmp/chassis-lab-venv/bin/python -m chassis_lab.part_assemblies --render-only
```

Open `part_design_runs/four_catalog_designs/gallery.html` to compare original
CAD and reduced display detail side by side. The four proposals are generated
by GPT-5.6 Sol at low reasoning, strictly choosing existing traction/omni wheel
definitions at the two driven and two passive axle positions. All other source
parts, including the arm/claw, retain their original placements. These are four
Clawbot drivetrain variants, **not four novel scoring mechanisms**.

Each folder contains `design.json`, `assembly.json`, a source `bom.json`, original
CAD and low-detail MJCF models, images, and `audit.json`. The shared
`part_catalog.json` links source definition IDs, original STEP, original and
reduced meshes, hashes, units, and unknown engineering fields. Same-number CAD
configurations are not silently merged. Wheel substitutions preserve the source
axle direction and width-center plane; they never stretch a mesh. Contact width
is derived from the selected wheel CAD; contact radii and friction remain the
documented approximations. Motor locations are preserved even when wheels swap.

Representation checks load both models and compare inertias, masses, joint axes,
motor bounds and timestep. They also reject LOD meshes used as collision geometry.
This mode does not step physics or run scoring/performance tests. Source CAD lacks
verified mates and a complete axle/fastener inventory; substitutions require
shaft-engagement, spacer-stack and clearance verification. Consequently these
models are **source-part assemblies, not certified buildable designs**. The
complete parts library and connection definitions are still needed for broader
mechanism generation. Unknown mass and mating metadata remain null rather than
being inferred from polygon detail.

## AI concept previews without simulation

```sh
/private/tmp/chassis-lab-venv/bin/python -m chassis_lab.concepts
# Re-render saved proposals without making API calls:
/private/tmp/chassis-lab-venv/bin/python -m chassis_lab.concepts --output concept_runs/sol20_preview --render-only
```

This separate preview mode requests twenty chassis + scoring concepts from
`gpt-5.6-sol` with `reasoning.effort=low`, using the authorized `.env.local` key.
Four batches of five carry previous ideas forward to encourage variety, with
at most one repair per batch, eight calls total and 10,000 output tokens per call.
It records AI responses, usage, structured designs, and a static MuJoCo MJCF/PNG
for every concept, plus `gallery.html` and `all_20.png`. No physics stepping,
controller trials, ranking, or performance evaluation occurs. These are primitive
concept illustrations, not imports of verified VEX part assemblies. The broader
preview schema intentionally permits architectures unsupported by the existing
simulation controller. Novelty and hardware feasibility are unverified.

## Imported CAD field and Clawbot driving test

The optional CAD pipeline preserves supplied STEP assemblies separately from
lightweight simulation assets. It currently ingests the user-provided Clawbot
STEP and Override field STEP ZIP into `data/cad_assets`, records source hashes
and assembly transforms, and tessellates each unique part once without destructive
whole-assembly decimation. It imports 65 Clawbot part definitions / 143 instances
and 71 field part definitions / 1,120 instances. Visual meshes have collision
disabled; explicitly separate proxies provide contact physics.

Create the one-time Python 3.11 conversion environment and import assets:

```sh
uv venv .venv-cad --python /opt/homebrew/bin/python3.11
uv pip install --python .venv-cad/bin/python -r requirements-cad.txt
.venv-cad/bin/python -c 'from pathlib import Path; from tools.cad_assets import canonical_sources; canonical_sources(Path.home()/"Downloads/claw bot.step", Path.home()/"Downloads/v5rc-override-fieldcad.zip")'
.venv-cad/bin/python -m tools.cad_tree clawbot
.venv-cad/bin/python -m tools.cad_tree override_field
.venv-cad/bin/python -m tools.cad_lod
```

Run the physical driving test, then inspect its recorded motion:

```sh
/private/tmp/chassis-lab-venv/bin/python -m chassis_lab cad-demo
/private/tmp/chassis-lab-venv/bin/mjpython -m chassis_lab mechanism-replay assemblies/cad_drive/cad_drive.xml
# Static assembly inspection:
/private/tmp/chassis-lab-venv/bin/python -m chassis_lab cad-view clawbot
/private/tmp/chassis-lab-venv/bin/mjpython -m chassis_lab cad-view clawbot --viewer
```

`cad-demo` now defaults to a small local field section (x: -1.5 to .55 m,
y: -1.5 to -.3 m in the imported field frame) and cached low-detail visuals.
Tiny decorative hardware is omitted, other visual parts use roughly 1,800
triangles or a convex display envelope, and goal/cup visuals retain their
openings. Shadows and multisample antialiasing are disabled; image export is
800 × 600. Canonical CAD, retained collision meshes, inertia, friction, motor
limits, solver and 2 ms timestep are unchanged. The crop has no artificial walls;
it is appropriate only for local tests that stay inside it. This does not yet
turn the fixed-arm drive test into a functional scoring benchmark.

Use `cad-demo --full-field` for the complete layout, and `--full-detail` for
original visual meshes (expensive). `cad-view` remains a detailed inspection tool.
Visual detail and field extent can be selected independently.
The full-versus-lightweight timing and trajectory comparison is saved in
`assemblies/cad_performance/comparison.json`; rerun it with
`python -m tools.profile_cad` after generating both comparison scenes.

Open `assemblies/cad_drive/report.html` for measured results, an animated replay,
and screenshots. `cad_drive.xml`, `cad_drive.npz`, `metrics.json`, and
`configuration.json` preserve the model, trajectory, measurements and assumptions.
Use `--no-render` for headless physics. Rendering failure is recorded separately.
The fixed 10-second program settles, drives, brakes, turns, brakes, and drives again.
Only two wheel motors actuate the free chassis; its root is never repositioned
during simulation. Four CAD wheel instances rotate at their measured centers.
The other two wheels are passive, with low isotropic friction approximating omni
rollers. Wheel contact priority overrides floor friction. Mass is an explicit
2.83 kg estimate; the .5 Nm / 200 rpm linear motor envelope is uncalibrated.

This is a driving integration test, not a manipulation/scoring experiment:
the imported arm/claw and field objects are fixed, and field contact uses convex
part proxies which can fill openings in cups and goals. Screws are visible but
not individually compliant joints. No CAD mates, material properties, or official
legality are inferred. The earlier mechanism-search commands still use their
separate primitive models; they do not yet optimize this imported Clawbot.

The full STEP remains unchanged. `data/cad_assets/manifest.json` records source
hashes; `assembly_tree.json` records named parts and transforms, with per-part OBJ
and JSON in `parts/`. Bounds are conservative transformed mesh bounding boxes,
not exact manufacturing dimensions. Legacy `visual.obj` and `metadata.json`
from the initial whole-mesh prototype are superseded by the assembly tree.
CAD conversion uses optional Python 3.11 dependencies and takes several minutes;
simulation uses the normal MuJoCo environment. The `/private/tmp` simulation
environment can disappear on reboot; installation below also supports `.venv`.

The first local component-index layer follows the supplied design discussion's
CAD/simulation/metadata separation. It extracts named STEP `PRODUCT` definitions
and part numbers without inventing missing properties:

```sh
.venv-cad/bin/python -m tools.vex_import discover data/cad_assets/clawbot/canonical.step --name clawbot
.venv-cad/bin/python -m tools.vex_import build-index
.venv-cad/bin/python -m tools.vex_import validate
```

Both supplied STEP files contain named product and assembly records. The index
is an inventory of supplied assemblies, not the complete VEX V5 parts library. A future Onshape
adapter can populate the same schema once the library source/credentials are
provided. Unknown metadata remains `null`, and official legality is not inferred
from presence in CAD.

CAD tests require the optional assets. Repeated runs on the same MuJoCo build
are checked within 1e-8 m final-position tolerance; cross-version/platform bitwise
reproducibility is not promised. The test includes a zero-actuation control,
straight movement, turning, and stability checks.
...
## September 2026 parts catalog and four mechanism previews

Open `data/official_catalog/index.html` for all 418 unique SKUs from the downloaded
official legal-parts list, and `data/official_catalog/cad_gallery.html` for 1,198
downloaded community Fusion CAD files (1,193 embedded thumbnails). The archives
include small hardware. Full SKU matching, STEP conversion and mating data remain
incomplete; see `data/official_catalog/README.md` for provenance and coverage.

Open `part_design_runs/four_new_mechanisms/gallery.html` for four distinct scoring
layout concepts produced with actual `gpt-5.6-sol` low-reasoning API calls:
four-bar fork, roller feeder, tilting tray and overhead claw. These are static CAD
layouts built by hand-authored templates from AI parameters, not build-ready or
physics-ready assemblies. No performance tests were run. Required parts and
rendered parts are recorded separately; missing geometry is explicitly labeled.

Re-render the saved proposals, without API calls:

```sh
/private/tmp/chassis-lab-venv/bin/python -m chassis_lab.catalog_concepts --render-only
```

The general low-detail CAD display now retains small hardware rather than hiding
parts under 20 mm. This display change does not introduce extra physical bodies,
contacts or joints.
